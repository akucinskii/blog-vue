...
@router.get("")
def read_users(skip: int = 0, limit: int = 25,  db: Session = Depends(get_db), user: models.User = Depends(crud_auth.get_current_active_user)):
...
@router.delete("")
def remove_user(user_id: UUID, db: Session = Depends(get_db), user: models.User = Depends(crud_auth.get_current_active_user)):
